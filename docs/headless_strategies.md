...
- 对于**体量较小/一次性**的简单站点，可以统一写在 `custom.py` 中，方便集中维护；
- 对于 Twitter/X、HuggingFace 等复杂站点，建议单独创建模块（如 `twitter.py`、`huggingface.py`）。
...
`@route` 的第一个参数 `pattern` 用于判断当前 URL 是否命中该策略，支持两种形式：

- 字符串：如 `"example.com"`，内部通过 `pattern in url` 的子串匹配判断是否命中；
- 函数：形如 `Callable[[str], bool]`，入参为完整 URL 字符串，返回 `True` 表示命中，例如：

  ```python
  @route(lambda url: url.startswith("https://sub.example.com/"))
  def process_sub_domain(page):
      ...
  ```
